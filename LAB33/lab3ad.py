...
if os.path.exists(df_all_path):
    ...
    for ids in range(1, 28):  
        ...
        if response.status_code == 200:
            file_name = f'vhi_id_{ids}_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.csv'
...
with st.sidebar:
    ...
    filters['time_series'] = st.selectbox("Oberit chasoviy ryad:", ['VCI', 'TCI', 'VHI'], index=['VCI', 'TCI', 'VHI'].index(filters['time_series']))
    filters['region'] = st.selectbox("Oberit oblast:", ordered_regions, index=ordered_regions.index(filters['region']))
    filters['week_range'] = st.slider("Oberit tyzhni:", int(df_all['Week'].min()), int(df_all['Week'].max()), filters['week_range'])
    filters['year_range'] = st.slider("Oberit roki:", int(df_all['Year'].min()), int(df_all['Year'].max()), filters['year_range'])
    filters['sort_asc'] = st.checkbox("Sortuvati za zrostannyam", value=filters['sort_asc'])
    filters['sort_desc'] = st.checkbox("Sortuvati za spadannyam", value=filters['sort_desc'])
...
if f['sort_asc'] and not f['sort_desc']:
    ...
    st.warning("Oberit lishe odin variant.")

#Вкладки для відображення результатів
tab1, tab2, tab3 = st.tabs(["Tablitsya", "Graphic", "Porivnyannya"])

#Таблиця
with tab1:
    st.subheader("Tablitsya")
    st.dataframe(filtered_df)

#Графік-теплокарта
with tab2:
    st.subheader("Graphic (heatmap)")
    if not filtered_df.empty:
        ...
        ax.set_title(f"Teplova karta {f['time_series']} для {f['region']} (po tizhnyah i rokah)")
        st.pyplot(fig)
    else:
        st.info("Nemaye danih dlya vidobrazhennya.")

#Порівняння по областях
with tab3:
    st.subheader("Porivnyannya po oblastyah")
    filtered_cmp = df_all[
        (df_all['Week'] >= f['week_range'][0]) & (df_all['Week'] <= f['week_range'][1]) &
        (df_all['Year'] >= f['year_range'][0]) & (df_all['Year'] <= f['year_range'][1])
    ]
    if not filtered_cmp.empty:
        ...
        ax.set_title(f"Seredniy {f['time_series']} po oblastyah za vibraniy period")
        ax.set_xlabel("Oblast")
        ax.set_ylabel(f"Seredniy {f['time_series']}")
        ax.set_xticklabels(grouped_cmp['area'], rotation=45, ha='right')
        ax.grid(axis='y')
        fig.tight_layout()
        st.pyplot(fig)
    else:
        st.info("Nedostatnyo danih dlya porivnyannya.")
